#!/usr/bin/env python
"""Generate an ElectroSense experiment report from saved OpenEW-SA artifacts."""
...
DEFAULT_DATASET_SUMMARY = Path(r"D:\openew_sa_data\tables\electrosense_dataset_summary.csv")
DEFAULT_MARKDOWN_OUTPUT = Path(r"D:\openew_sa_data\tables\electrosense_results_summary.md")
DEFAULT_CSV_OUTPUT = Path(r"D:\openew_sa_data\tables\electrosense_results_summary.csv")

EXPERIMENTS = (
    {
        "name": "electrosense_psd_mlp",
        "model": "Tabular MLP",
        "split_protocol": "Random row split across ElectroSense PSD samples.",
        "metrics_arg": "random_mlp_metrics",
    },
    {
        "name": "electrosense_sensor_holdout_mlp",
        "model": "Tabular MLP",
        "split_protocol": "Hold out sensors alcorcon1, bcn-L, and Geneva.",
        "metrics_arg": "sensor_holdout_mlp_metrics",
    },
)
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Create Markdown and CSV summaries for ElectroSense OpenEW-SA experiments.",
        formatter_class=argparse.ArgumentDefaultsHelpFormatter,
    )
    parser.add_argument("--dataset-summary", default=DEFAULT_DATASET_SUMMARY, type=Path)
    parser.add_argument(
        "--random-mlp-metrics",
        default=Path("runs") / "electrosense_psd_mlp" / "metrics.json",
        type=Path,
    )
    parser.add_argument(
        "--sensor-holdout-mlp-metrics",
        default=Path("runs") / "electrosense_sensor_holdout_mlp" / "metrics.json",
        type=Path,
    )
    ...
    dataset_summary = _read_csv(args.dataset_summary, "ElectroSense dataset summary")
...
def _dataset_info(dataset_summary: pd.DataFrame) -> dict[str, Any]:
    if dataset_summary.empty:
        raise ValueError("ElectroSense dataset summary is empty")
    ...
    labels = _read_json(artifact_dir / "labels.json", "ElectroSense labels.json")
...
def _write_markdown(dataset_info: dict[str, Any], experiment_table: pd.DataFrame, output: Path) -> None:
    output.parent.mkdir(parents=True, exist_ok=True)
    lines = [
        "# ElectroSense Experiment Results",
        "",
        "## Dataset",
        "",
        _markdown_table(
