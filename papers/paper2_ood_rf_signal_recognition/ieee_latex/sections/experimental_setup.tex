...
Each processed artifact directory contains \compactmethod{metadata.csv}, \compactmethod{labels.json}, and a shared NumPy feature array when available. A unified Paper 2 manifest stores \compactmethod{sample_id}, dataset, task, label, domain, input type, feature path, and feature index. Protocol-specific split CSVs preserve these columns and append the split and OOD label. The evaluation file contains test-ID and test-OOD rows, matching the prediction and score files one-to-one by \compactmethod{sample_id}.
