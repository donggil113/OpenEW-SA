...
In the current Paper~1 datasets, \artifact{sample\_id}, \artifact{dataset\_source}, \artifact{input\_type}, \artifact{domain\_id}, and task-specific labels are actively populated. JamShield uses \artifact{abnormal\_event\_label}, DeepSense uses \artifact{occupancy\_label}, and ElectroSense uses \artifact{situation\_label}. Fields such as \artifact{synthetic\_mission\_context}, \artifact{threat\_level}, and \artifact{human\_review\_required} are reserved for future RF situation-awareness extensions and are not used as predictive labels in the experiments reported here.
...
The paper reports experiments generated through the current OpenEW-SA repository workflow and does not redistribute raw datasets. Additional RF tasks, stronger architectures, and new preprocessing variants are left for future benchmark releases. Raw datasets are not stored in the repository.
...
The OpenEW-SA code is maintained at \url{https://github.com/donggil113/OpenEW-SA}. The repository contains conversion, training, evaluation, reporting, and LaTeX-generation scripts. Raw datasets are not redistributed with the repository and must be downloaded from their original public sources. Local raw-data, processed-data, run-output, and table paths are configured through YAML files. No Zenodo DOI is claimed for the current version.
