...
Progress in this area is constrained by data availability. Real military EW data are rarely public because they can contain sensitive platforms, waveforms, locations, tactics, or collection capabilities. As a result, reproducible research must often rely on public RF datasets that were not originally designed as a unified EW benchmark. These datasets are valuable, but they are heterogeneous: JamShield provides tabular WiFi metrics for benign and jamming conditions [REF: JamShield], DeepSense SDR WiFi provides raw I/Q captures with occupancy labels [REF: DeepSense], and ElectroSense provides PSD traces over distributed sensors and spectrum technologies [REF: ElectroSense].
...
RF machine learning benchmarks commonly focus on specific tasks such as modulation recognition, spectrum sensing, jamming detection, RF fingerprinting, or transmitter identification [REF: RF machine learning benchmarks]. Modulation recognition datasets have been especially influential for I/Q representation learning [REF: modulation recognition datasets], while spectrum sensing datasets provide occupancy and sensing tasks closer to spectrum-monitoring workflows [REF: spectrum sensing datasets]. RF fingerprinting benchmarks further emphasize device identity and receiver-domain effects [REF: RF fingerprinting]. For spectrum situation awareness, however, task performance alone is insufficient. A useful benchmark must also expose whether a model generalizes across operational domains.

Spectrum sensing and occupancy datasets provide natural domain shifts because sensors, days, and locations often correspond to different propagation and hardware conditions [REF: spectrum sensing datasets]. DeepSense-style SDR captures expose raw I/Q windows and collection-day variation [REF: DeepSense]. ElectroSense-style PSD measurements expose receiver and location effects across frequency-domain observations [REF: ElectroSense]. Jamming and interference datasets such as JamShield add scenario and jammer-family shifts, which are especially relevant to abnormal-event detection [REF: JamShield].

Domain generalization is a recurring challenge in RF learning [REF: RF domain generalization]. Models can exploit receiver artifacts, collection conditions, or scenario-specific signatures that do not transfer to new domains. Random sample splits may therefore conflate within-domain interpolation with out-of-domain generalization. OpenEW-SA is designed to make that distinction explicit by reporting both random row-level baselines and domain-aware holdouts.

The benchmark also connects to neuro-symbolic reasoning [REF: neuro-symbolic reasoning] and graph/hypergraph situation awareness [REF: graph/hypergraph situation awareness]. Receivers, domains, bands, time windows, labels, and threat states can be treated as typed entities in a graph or hypergraph. Before such models can be evaluated, the underlying data must be converted into consistent artifacts with comparable metadata. OpenEW-SA provides that conversion and evaluation layer.
...
- [REF: RF machine learning benchmarks] Add survey or benchmark references covering RFML tasks and evaluation practice.
- [REF: modulation recognition datasets] Add references for public modulation-recognition datasets, including RadioML-style baselines.
- [REF: spectrum sensing datasets] Add references for public spectrum-sensing and occupancy datasets.
- [REF: DeepSense] Add the primary DeepSense SDR WiFi or spectrum-sensing dataset citation.
- [REF: JamShield] Add the primary JamShield dataset citation.
- [REF: ElectroSense] Add the primary ElectroSense dataset citation.
- [REF: RF domain generalization] Add references on domain shift and generalization in RF machine learning.
- [REF: RF fingerprinting] Add references on RF fingerprinting datasets and receiver/device-domain effects.
- [REF: neuro-symbolic reasoning] Add references on neuro-symbolic reasoning methods relevant to situation awareness.
- [REF: graph/hypergraph situation awareness] Add references on graph or hypergraph models for dynamic situation awareness.
