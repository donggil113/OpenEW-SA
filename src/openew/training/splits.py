...
def build_holdout_split_indices(metadata: pd.DataFrame, config: dict[str, Any]) -> tuple[list[int], list[int]] | None:
    ...
    if strategy == SPLIT_DOMAIN_HOLDOUT:
        holdout_mask = _domain_holdout_mask(metadata, config.get("holdout_domains", []))
    else:
        holdout_mask = _jammer_type_holdout_mask(metadata, config.get("holdout_jammer_types", []))
...
def _domain_holdout_mask(metadata: pd.DataFrame, holdout_domains: list[str]) -> pd.Series:
    ...
    holdout_set = {str(domain) for domain in holdout_domains}
    return metadata["domain_id"].fillna("").astype(str).isin(holdout_set)


def _jammer_type_holdout_mask(metadata: pd.DataFrame, holdout_jammer_types: list[str]) -> pd.Series:
    if "domain_id" not in metadata.columns:
    ...
    return metadata["domain_id"].map(infer_jammer_type).isin(holdout_set)
